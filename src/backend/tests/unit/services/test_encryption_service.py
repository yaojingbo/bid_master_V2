...
class TestEncryptionService:
    ...
    def test_generate_key_produces_valid_fernet_key(self):
        """generate_key 应产生有效的 Fernet 密钥（44 字符 base64）。"""
    ...
    def test_key_from_password_produces_valid_key(self):
        ...
        key = EncryptionService.key_from_password(password, salt)
        assert isinstance(key, bytes)
        assert len(key) == 44  # Fernet key is 44 bytes when base64 encoded
    ...
    def test_key_from_password_same_inputs_same_key(self):
        ...
        password = "test_password_123"
        salt = b"salt_value_12345"
        key1 = EncryptionService.key_from_password(password, salt)
        key2 = EncryptionService.key_from_password(password, salt)
        assert key1 == key2

    def test_key_from_password_different_salt_different_key(self):
        """不同盐应产生不同密钥。"""
        password = "test_password_123"
        key1 = EncryptionService.key_from_password(password, b"salt_1")
        key2 = EncryptionService.key_from_password(password, b"salt_2")
        assert key1 != key2
